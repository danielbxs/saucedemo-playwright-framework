...
test.describe("Inventory Functionality", () => {
  ...
  test("should be redirected to new website when clicking about link on menu", async ({
    page,
  }) => {
  ...
  test.fail("should open side menu and reset app state", async () => {
    ...
    await expect(pm.onInventoryPage().shoppingCartBadge).not.toBeVisible();
    await expect(
      pm.onInventoryPage().getAddToCartButton("Sauce Labs Bike Light"),
    ).toHaveText("Add to cart");
    await expect(
      pm.onInventoryPage().getAddToCartButton("Sauce Labs Fleece Jacket"),
    ).toHaveText("Add to cart");
  });
